...
networkDesc_t* ZDApp_NwkDescListProcessing(void)
{
  ...
  //char cStr[64] = {""};
  //sprintf(cStr,"--- ZDApp: nwk_getNwkDescList. PanID = 0x%x\r\n", pNwkDesc->panId);
  ...
  while (pNwkDesc)
  {
	...
	//sprintf(cStr,"--- ZDApp: nwk_getNwkDescList. Counter = %d\r\n", ResultCount);
  //DBG_PRINTF((uint8 *)cStr);
  ...
  if ( i == ResultCount )
  {
	  //DBG_PRINTF("couldn't find appropriate PAN to join\n");
    return (NULL);   // couldn't find appropriate PAN to join !
  }
  else
  {
	   //DBG_PRINTF("PAN to join OK\n");
    return (pNwkDesc);
  }
