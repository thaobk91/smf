...
function cGetcUSD(strCUSD)
	...
	--print("---- cGetcUSD 1:", cMainAcc, "\r\n");
	cMainAcc = string.gsub(strCUSD,"\r","");
...
function handle_received_sio_data(hdr_idx)
  if(hdr_idx == SIO_ATI_RECVED) then
    ...
    return;
  elseif(hdr_idx == SIO_CUSD) then
   print("---- handle_received_sio_data: Getting cUSD response....\r\n");
   cData_publish = cGetcUSD(sio_recved_string);  
  ...
  while (true) do
    ...
    if(evt == TIMER_EVENT) then
      ...
      elseif(evt_param == TIMER_ID_CHECKFLAG) then
        if( (flag_mqtt_cfg == 1) or ((flag_get_idgw == 2) and (flag_cntgetid > 10)) ) then       -- has msg config mqtt: server, port, user, pass.
          mqtt_state = MQTT_STATE_DISCONNECT;
          next_state = true;
          flag_mqtt_cfg = 0;
          flag_get_idgw = 1;
...
--            vmstoptimer(TIMER_ID_GETIDGW);
      ...
      elseif(evt_param == TIMER_ID_MQTT) then               -- manage  MQTT connect, sub
        if((next_state == true) and (flag_start == 1) and (flag_get_idgw > 0)) then  -- ensure get id done, sim ready
          vMQTT_Process();
        end;
...
SIO_CUSD                = 19;
SIO_ERROR_RECVED                    = 20;           -- remember put OK in end pos
...
sio_rpt_hdr[SIO_CUSD]                             = "+CUSD";           -- data gps from gps
------------------------------------------------------------------------------
------------------------------------------------------------------------------
--define state MQTT
MQTT_STATE_INIT             = 0;
...
print("\r\n-------------------3G_GW Version 1.1 Last Update 18-Oct-2018-----------------------\r\n");
sio.send("ATE0\r\n");   -- echo off
vmsleep(200);
sio.send("AT+CTZU=1\r\n");  -- turn on auto update time via NITZ
vmsleep(200);
sio.send(cSet_Baurate);
------------------------------------------------------------------------------
-- include file
