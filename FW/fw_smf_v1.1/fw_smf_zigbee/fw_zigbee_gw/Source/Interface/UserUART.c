...
#define macroUART_DEBUG_IOC_PORT				GPIO_B_BASE
#define macroUART_DEBUG_RX_PIN					GPIO_PIN_1
#define macroUART_DEBUG_TX_PIN					GPIO_PIN_2
...
#define macroUART_APP_IOC_PORT					GPIO_C_BASE
#define macroUART_APP_RX_PIN					GPIO_PIN_5
#define macroUART_APP_TX_PIN					GPIO_PIN_4
...
void vUART_Init(uint32 UART_Base, uint32 ulBaudrate)
{
	...
	if(UART_Base == macroUART_DEBUG_BASE)
	{
	...
	else if(UART_Base == macroUART_APP_BASE)
	{
	...
	if(UART_Base == macroUART_APP_BASE)
	{
...
{
	uint16 ui = 0;
	APP_DEBUG("--- UserUART: uart app write \"%s\"\r\n", Buffer);
	while(ui <= strlen(Buffer))
	{
